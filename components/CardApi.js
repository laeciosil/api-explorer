...
function CardApi({ api, numberLength }) {
  const {setApiDetails} = useContext(AppContext);
  const router = useRouter();

  function handleApiDetails(api) {
    setApiDetails(api);
    router.push(`/apidetails/${api.id}`);
  }

  return (
    <div
      onClick={() => handleApiDetails(api)}
    ...
    >
      ...
      <p className="text-[#979899] text-base">
        {api.description.length > numberLength
          ? `${api.description.slice(0, numberLength)}...`
          : api.description}
      </p>
