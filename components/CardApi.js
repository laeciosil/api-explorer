...
function CardApi({api}) {
  return (
    <div className="mr-2 p-4 bg-light-primary rounded-md dark:bg-dark-primary shadow-lg ring-1 ring-black ring-opacity-5 space-y-1">
      ...
      <p className="text-[#979899] text-base">{api.description}</p>
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center">
          ...
          <span className="bg-[#e4e5e9] text-light-secondary text-xs font-bold  px-2.5 py-0.5 rounded dark:text-dark-secondary ml-3">{`${api.rating}.0`}</span>
        </div>
      </div>
    </div>
  );
